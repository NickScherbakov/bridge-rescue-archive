...
const BOUNTIES = [
    ...
    },
    {
        ...
        reward: 3000,
        client: 'ИП "Орлов и Партнеры" (Селлер на 3-х площадках)',
        desc: 'Крупнейший маркетплейс молча обновил API ночью в субботу. Python-шлюз начал возвращать пустые массивы при ошибке, что привело к массовому списанию упаковки в 1С:УТ без товара. Остановите шлюз.',
        status: 'ОТКРЫТ',
        solved: false,
        code: [
            '# ФРАГМЕНТ ШЛЮЗА: marketplace_gateway.py',
            'def fetch_orders(session, marketplace_id):',
            '    try:',
            '        resp = session.get(f"/api/v3/orders/{marketplace_id}")',
            '        resp.raise_for_status()',
            '        return resp.json()["orders"]',
            '    except KeyError:',
            '        order_list = []   # <-- тихое проглатывание: пустой список вместо ошибки',
            '        return order_list',
        ],
        iooHint: [
            'Маркетплейс не предупреждает. Он просто меняет схему.',
            'Тихое проглатывание KeyError — вот корень каскада.',
            'Пустой список попал в 1С:УТ и был воспринят как «товар отгружен».',
            'Шлюз должен поднять исключение или явно вернуть ошибку.',
            'Добавь raise или return с признаком ошибки — остановишь цепочку.',
        ],
    },
];
...
async function cmdPatch(args) {
    ...
    // Проверка патча в зависимости от контракта
    let hasCheck = false;
    let rejectHint = '';

    if (id === '1C-882') {
        hasCheck = /БазаРаспределения\s*>\s*0|Если.*БазаРаспределения.*>\s*0/i.test(userCode);
        rejectHint = 'Ожидается условие: «Если БазаРаспределения > 0 Тогда»';
    } else if (id === 'API-MKT-709') {
        hasCheck = /\breturn\b|\braise\b/i.test(userCode);
        rejectHint = 'Ожидается явный raise или return с признаком ошибки вместо тихого проглатывания.';
    } else {
        hasCheck = userCode.length > 0;
        rejectHint = 'Патч не прошёл проверку.';
    }
    ...
    if (id === 'API-MKT-709') {
        // Стилизованный крипто-чек для маркетплейсного контракта
        print('│  ╔══════════════════════════════════════════════╗', 'amber');
        print('│  ║        SYNAPSE_TOKEN  RECEIPT  #MKT709       ║', 'amber');
        print('│  ╠══════════════════════════════════════════════╣', 'amber');
        print(`│  ║  ОТ   : SAFE_HAVEN NODE v0.9                 ║`, 'amber');
        print(`│  ║  КОМУ : Архитектор                           ║`, 'amber');
        print(`│  ║  ЗА   : Остановка шлюза API-MKT-709          ║`, 'amber');
        print(`│  ║  СУММА: +${contract.reward} SYNAPSE_TOKENS            ║`, 'amber');
        print(`│  ║  ХЭШ : 0x${(contract.id + Date.now().toString(16)).replace(/[^0-9A-Fa-f]/g,'').slice(0,8).toUpperCase()}...BRIDGE          ║`, 'amber');
        print('│  ╚══════════════════════════════════════════════╝', 'amber');
        print('│  Шлюз остановлен. Каскад прерван. Склад в порядке.', 'success');
    } else {
    ...
    if (id === 'API-MKT-709') {
        await iooSay([
            'Тихое проглатывание устранено. Ошибка теперь видна.',
            'Маркетплейс не предупреждает — но система предупредит.',
