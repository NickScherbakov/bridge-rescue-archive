...
const BOUNTIES = [
    ...
    },
    {
        ...
        patchErrMsg: 'Ожидается: return или raise вместо тихого проглатывания.',
        completionMsg: '[СИСТЕМА] Трафик в 1С:УТ остановлен. Пустые реализации отменены.',
        showCryptoTransfer: true,
        completionIoo: [
...
async function cmdPatch(args) {
    ...
    if (contract.showCryptoTransfer) {
        print('│', 'success');
        print('│  ╔══════════════════════════════════════╗', 'amber');
    ...
    if (contract.completionIoo) {
        await iooSay(contract.completionIoo);
    }
    printEmpty();
}
